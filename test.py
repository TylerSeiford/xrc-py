...
class GameElementState:
    ...
    @staticmethod
    def read() -> 'GameElementState':
        '''Returns the current state of the game'''
        with open('GameElements.txt', 'r', encoding='UTF+8') as file:
            raw = file.read()
        raw = raw.strip()
        raw = json.loads(raw)
        elements = []
...
class RobotState:
    ...
    def read() -> 'RobotState':
        ...
        with open('myRobot.txt', 'r', encoding='UTF+8') as file:
            raw = file.read()
        raw = raw.strip()
        raw = json.loads(raw)
        elements = []
...
class GameState:
    ...
    def read() -> 'GameState':
        '''Returns the current state of the game'''
        with open('GAME_STATE.txt', 'r', encoding='UTF+8') as file:
            phase = file.readline()
...
class Gamepad:
    ...
    def read(self) -> Controls:
        ...
        trigger_left = 0
        trigger_right = 0
        return Controls(a, b, x, y, dpad_down, dpad_up, dpad_left, dpad_right,
                bumper_left, bumper_right, stop, restart,
                right_y, right_x, left_y, left_x, trigger_left,
                trigger_right)
...
if __name__ == '__main__':
    ...
    while True:
        # Read gamepad, read game data and write to file
        controls = gamepad.read()
        try:
            game_state = GameState.read()
            element_state = GameElementState.read()
            robot_state = RobotState.read()
